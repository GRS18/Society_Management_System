...
export default function ContactMessages() {
  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get("http://localhost:8089/contact-messages/all");
  ...
  const markAsResolved = async (id) => {
    try {
      await axios.put("http://localhost:8089/contact-messages/update/${id}", { status: "resolved" });
  ...
  const deleteMessage = async (id) => {
    ...
    try {
      await axios.delete("http://localhost:8089/contact-messages/delete/${id}");
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-center">Contact Messages</h2>
